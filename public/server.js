  ...
  (req, res) => {
    ...
    centers.set(
      centerKey,
      {
        ...
        weeklyRankingAll:
          Array.isArray(body.weeklyRankingAll)
            ? body.weeklyRankingAll
            : Array.isArray(body.weeklyRanking) ? body.weeklyRanking : current.weeklyRankingAll || [],
...
app.get(
  ...
  (req, res) => {
    ...
    const list =
      Array.isArray(center?.weeklyRankingAll) && center.weeklyRankingAll.length
        ? center.weeklyRankingAll
        : Array.isArray(center?.weeklyRanking) ? center.weeklyRanking : [];

    // 주간 집계 이후 접수된 관제 값으로 오늘 실적 증가분을 반영한다.
    const freshLive = center.receivedAt && center.weeklyUpdatedAt &&
      Date.parse(center.receivedAt) >= Date.parse(center.weeklyUpdatedAt);
    const live = freshLive && Array.isArray(center.riders) ? center.riders : [];
    const snapshot = Array.isArray(center.todayDetails) ? center.todayDetails : [];
    const liveById = new Map(live.filter(r => r.userId).map(r => [String(r.userId), r]));
    const snapshotById = new Map(snapshot.filter(r => r.userId).map(r => [String(r.userId), r]));
    const updated = list.map(r => {
      const key = String(r.userId || "");
      const current = liveById.get(key), previous = snapshotById.get(key);
      return current && previous
        ? { ...r, val: Math.max(0, (Number(r.val) || 0) - (Number(previous.val) || 0) + (Number(current.allDayComplete) || 0)) }
        : r;
    }).sort((a, b) => (Number(b.val) || 0) - (Number(a.val) || 0)).slice(0, 5);
...
const list =
  Array.isArray(center.todayDetails)
    ...
    // 최신 관제 라이더 값이 있으면 주간 동기화 전에도 오늘 실적을 표시한다.
    const live = Array.isArray(center.riders)
      ? center.riders.find(r => String(r.userId || "").trim() === riderUserId)
      : null;
    if (!rider && !live) {
      return res.status(404).json({
        ok: false,
    ...
    if (!rider) rider = { name: live.name, userId: live.userId };
    const liveIsCurrent = live && (!center.weeklyUpdatedAt ||
      Date.parse(center.receivedAt || 0) >= Date.parse(center.weeklyUpdatedAt));
    const peaks = liveIsCurrent
      ? (live.deliveryPeakTimeCount || {
          morning: live.morning, afternoon: live.afternoon,
          evening: live.evening, midnight: live.night
        }) : null;
    ...
    res.json({
      ...
      data: {
        name: rider.name,
        userId: rider.userId,
        total: liveIsCurrent ? Number(live.allDayComplete) || 0 : Number(rider.val) || 0,
        food: liveIsCurrent && live.foodComplete != null ? Number(live.foodComplete) || 0 : Number(rider.food) || 0,
        bmart: liveIsCurrent && live.bmartComplete != null ? Number(live.bmartComplete) || 0 : Number(rider.bmart) || 0,
        store: liveIsCurrent && live.storeComplete != null ? Number(live.storeComplete) || 0 : Number(rider.store) || 0,
        out: liveIsCurrent && live.slaOutComplete != null ? Number(live.slaOutComplete) || 0 : Number(rider.out) || 0,
        ...(peaks ? { deliveryPeakTimeCount: peaks } : {}),
        ...(liveIsCurrent && Array.isArray(live.hourlyCompleted) && live.hourlyCompleted.length ? { hourlyCompleted: live.hourlyCompleted } : {})
      }
    });
