...
function detailRows() {
  ...
  if (myToday) {
    ...
    byDate.set(key, { ...existing, ...myToday, date: key, deliveryPeakTimeCount: myToday.deliveryPeakTimeCount ?? existing.deliveryPeakTimeCount, hourlyCompleted: myToday.hourlyCompleted ?? existing.hourlyCompleted });
  }
  return [...byDate.values()].sort((a,b)=>String(a.date).localeCompare(String(b.date)));
}
...
function formatKoreanDate(date, withYear = true) {
  ...
  return `${withYear ? `${String(date.getFullYear()).slice(-2)}년 ` : ""}${date.getMonth() + 1}월 ${date.getDate()}일 (${weekday})`;
}
...
function renderRecentMonths() {
  ...
  box.innerHTML=`<button type="button" class="recent-label" disabled>최근 3개월</button>`+months.map(d=>`<button type="button" data-month="${dateKey(d).slice(0,7)}" class="${d.getFullYear()===calendarDate.getFullYear()&&d.getMonth()===calendarDate.getMonth()?"active":""}">${String(d.getFullYear()).slice(-2)}년 ${d.getMonth()+1}월</button>`).join("");
...
function renderHourly(row, key="") {
  ...
  if(!items.length && row && key && detailHourlyCache.has(key)) items=detailHourlyCache.get(key);
  if(!items.length){const empty=`<div class="daily-hourly-empty">선택한 날짜의 배달 기록이 없습니다.</div>`; if(box.innerHTML!==empty)box.innerHTML=empty; if(note)note.classList.add("hidden"); return;}
  const max=Math.max(1,...items.map(x=>x.value));
  const markup=`<div class="daily-hourly-track">${items.map(x=>`<div class="hourly-item"><strong>${x.value>0?x.value:""}</strong><i class="${x.value>0?"":"zero"}" style="height:${x.value>0?Math.max(4,x.value/max*92):2}px"></i><span>${x.label}</span></div>`).join("")}</div>`;
  if(box.innerHTML!==markup)box.innerHTML=markup;
  if(note)note.classList.add("hidden");
}

function renderDailyPeaks(row, key="") {
  ...
  if(!has && row && key && detailPeakCache.has(key)){ peak=detailPeakCache.get(key); has=true; }
  keys.forEach(([id,k])=>setText(id,has?`${Number(peak[k])||0}건`:"-"));
  const note=$("dailyPeakNote"); if(note){note.textContent=has?"":"선택한 날짜의 배달 기록이 없습니다.";note.classList.toggle("hidden",has);}
}
...
function renderDailyDetail() {
  ...
  if(key===bizKey&&myToday){ row={...(row||{}),...myToday,date:key,deliveryPeakTimeCount:myToday.deliveryPeakTimeCount??row?.deliveryPeakTimeCount,hourlyCompleted:myToday.hourlyCompleted??row?.hourlyCompleted}; }
  const p=rowParts(row);
  ...
  renderHourly(p.total>0?row:null,key); renderDailyPeaks(p.total>0?row:null,key);
...
function renderPeriodDetail() {
  ...
  setText("periodRangeTitle", `${String(periodStartDate.getFullYear()).slice(-2)}년 ${periodStartDate.getMonth()+1}월 ${periodStartDate.getDate()}일 ~ ${String(periodEndDate.getFullYear()).slice(-2)}년 ${periodEndDate.getMonth()+1}월 ${periodEndDate.getDate()}일`);
