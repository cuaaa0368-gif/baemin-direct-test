...
function buildLivePayload(snapshot) {
  ...
  const riders = list.map(d => ({
    ...
    bmartComplete: safe(d.deliveryAcceptanceCount?.bmartComplete),
    storeComplete: safe(d.deliveryAcceptanceCount?.storeComplete),
    slaOutComplete: safe(d.deliveryAcceptanceCount?.slaOutComplete),
    deliveryPeakTimeCount: d.deliveryPeakTimeCount ? { ...d.deliveryPeakTimeCount } : null,
    hourlyCompleted: Array.isArray(d.hourlyCompleted) && d.hourlyCompleted.length ? d.hourlyCompleted : null,
...
function buildWeeklyPayload(week) {
  ...
  const weeklyRankingAll = [...weeklyRankingMap.values()]
    .sort((a, b) => b.val - a.val);
  const weeklyRanking = weeklyRankingAll.slice(0, 5);
