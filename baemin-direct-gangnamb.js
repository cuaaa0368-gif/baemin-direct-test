...
const CENTER_ID = String(process.env.BAEMIN_GANGNAMB_CENTER_ID || "DP2609083866").trim();
const INITIAL_COOKIE = String(process.env.BAEMIN_COOKIE || "").trim();
const CENTER_KEY = "gangnamb";
const CENTER_NAME = "강남B";

const LIVE_MS = Math.max(
  30_000,
  Number(process.env.BAEMIN_GANGNAMB_POLL_MS || process.env.BAEMIN_POLL_MS || 30_000) || 30_000
);

const WEEKLY_MS = Math.max(
  30_000,
  Number(process.env.BAEMIN_GANGNAMB_WEEKLY_MS || process.env.BAEMIN_WEEKLY_MS || 30_000) || 30_000
);

const CENTER_CHECK_MS = Math.max(
  60_000,
  Number(process.env.BAEMIN_GANGNAMB_CENTER_CHECK_MS || process.env.BAEMIN_CENTER_CHECK_MS || 5 * 60_000) || 5 * 60_000
);

const RUNNER_OFFSET = Number(process.env.BAEMIN_GANGNAMB_RUNNER_OFFSET || process.env.BAEMIN_RUNNER_OFFSET || 0) || 0;
const HISTORY_CACHE_MS = Math.max(
  60_000,
  Number(process.env.BAEMIN_GANGNAMB_HISTORY_CACHE_MS || process.env.BAEMIN_HISTORY_CACHE_MS || 5 * 60_000) || 5 * 60_000
);
const HISTORY_ON_START = String(process.env.BAEMIN_GANGNAMB_HISTORY_ON_START || process.env.BAEMIN_HISTORY_ON_START || "1") !== "0";
const DIRECT_ENABLED = String(process.env.BAEMIN_GANGNAMB_DIRECT_ENABLED || "1") !== "0";
...
async function startBaeminDirectCollector({ port, ingestKey }) {
  ...
  if (!DIRECT_ENABLED) {
    console.log("[BAEMIN DIRECT GANGNAMB] disabled by BAEMIN_DIRECT_ENABLED=0");
    return getBaeminDirectStatus();
  }

  if (!CENTER_ID || !INITIAL_COOKIE) {
    console.log(
      "[BAEMIN DIRECT GANGNAMB] 환경변수 미설정 - 기존 Tampermonkey ingest 호환 모드로 실행 " +
  ...
  console.log(
    `[BAEMIN DIRECT GANGNAMB] START center=${CENTER_KEY}/${CENTER_NAME} interval=${LIVE_MS}ms`
  );
  console.log(`[BAEMIN DIRECT GANGNAMB] cookie names=${state.cookieNames.join(",")}`);
